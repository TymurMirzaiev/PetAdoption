...
public class PetTypesAdminController : ControllerBase
{
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> Update(Guid id, UpdatePetTypeRequest request)
    {
        await _mediator.Send(new UpdatePetTypeCommand(id, request.Name));
        return Ok();
    }
    ...
    {
        await _mediator.Send(new DeactivatePetTypeCommand(id));
        return Ok();
    }
    ...
    {
        await _mediator.Send(new ActivatePetTypeCommand(id));
        return Ok();
    }
}
